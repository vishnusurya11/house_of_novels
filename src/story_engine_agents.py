#!/usr/bin/env python3
"""
Multi-Agent Story Engine Prompt Generator

Uses a team of 4 AI agents to debate and select cards for story prompts,
...
def save_output(prompts: list[dict], metadata: list[dict]) -> Path:
    """
    Save generated prompts to JSON file with timestamp.
    ...
    output_path = output_dir / f"story_prompts_{timestamp}.json"
...
def main():
    """Run multi-agent prompt generation for all prompt types."""
    print("\n" + "#" * 60)
    print("# MULTI-AGENT STORY ENGINE")
    ...
    for config_name, config_class in PROMPT_CONFIGS.items():
        config = config_class()
