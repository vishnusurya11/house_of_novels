...
def run_template1_generation(
    ...
    steps_to_run = steps if steps is not None else [0, 1, 2, 3, 4, 5]
    ...
    # =========================================================================
    # Step 4: Chapter Title Cards (E3 template approach: ONE base + Qwen Edit per chapter)
    # =========================================================================
    if 4 in steps_to_run:
        step_start = time.time()
        card_workflow = str(get_workflow_path("chapter_title_card"))
        edit_workflow = str(get_workflow_path("scene_location_edit"))
        phase3_metadata["workflows_used"]["chapter_title_card"] = Path(card_workflow).name

        print(f"\n{'='*60}")
        print("STEP 4: Chapter Title Cards (Template Approach)")
        print(f"  Template workflow: {Path(card_workflow).name}")
        print(f"  Edit workflow: {Path(edit_workflow).name}")
        print(f"{'='*60}")
        ...
        # Get shared template prompt (generated by Phase 2 Step 7)
        template = chapters_data.get("chapter_card_template", {})
        template_prompt = template.get("background_prompt", "")

        if not template_prompt:
            print(">>> No chapter card template prompt found, skipping")
            print("    (Run Phase 2 Step 7 first)")
        else:
            # Pass 1: Generate ONE shared template image
            template_prefix = f"api/{timestamp}/chapter_cards/template"
            print(f"\n    Pass 1: Generating shared template image...")

            success, gen_data = _generate_image(
                template_prompt,
                template_prefix,
                "chapter_card_template",
                workflow_path=card_workflow,
                comfyui_url=comfyui_url,
                timeout=timeout,
            )

            template["base_generation"] = gen_data
            ...
            if success is None:
                ...
                print(f"    Template image failed, skipping chapter cards")
            else:
                template_path = _find_comfyui_output(template_prefix)
                if template_path is None:
                    print(f"    Template image output not found")
                else:
                    template["template_image_path"] = str(template_path)
                    print(f"    Template: {template_path.name}")

                    # Pass 2: Qwen Edit per chapter (using shared template)
                    chapters_with_edits = [
                        ch for ch in chapters
                        if ch.get("chapter_title_card", {}).get("qwen_edit_prompt")
                    ]

                    if not chapters_with_edits:
                        print("    No per-chapter edit prompts found")
                    else:
                        print(f"\n    Pass 2: Qwen Edit for {len(chapters_with_edits)} chapters...")

                        for ch in chapters_with_edits:
                            ch_num = ch.get("chapter_number", 0)
                            ch_title = ch.get("chapter_title", f"Chapter {ch_num}")
                            card_data = ch["chapter_title_card"]
                            qwen_prompt = card_data["qwen_edit_prompt"]

                            edit_prefix = f"api/{timestamp}/chapter_cards/ch{ch_num:02d}_final"
                            print(f"      Ch{ch_num}: {ch_title}...")

                            edit_success, edit_gen = _generate_location_layer(
                                prompt=qwen_prompt,
                                base_image_name=str(template_path),
                                filename_prefix=edit_prefix,
                                label=f"Ch{ch_num} card edit",
                                workflow_path=edit_workflow,
                                comfyui_url=comfyui_url,
                                timeout=timeout,
                            )

                            card_data["generation"] = edit_gen

                            if edit_success is None:
                                print(f"\n>>> ERROR: Cannot connect to ComfyUI at {comfyui_url}")
                                save_codex(codex, codex_path)
                                return _make_error_result(
                                    codex_path, f"Cannot connect to ComfyUI: {edit_gen.get('error', 'unknown')}", **_counts()
                                )
                            elif edit_success:
                                chapter_card_count += 1
                            else:
                                # Edit failed — fall back to template image
                                print(f"        Edit failed, using template image")
                                card_data["generation"] = {
                                    "status": "completed",
                                    "output_path": str(template_path),
                                    "generated_at": datetime.now().isoformat(),
                                    "note": "edit_failed_template_used",
                                }
                                chapter_card_count += 1

                    print(f"\n>>> Chapter cards complete: {chapter_card_count}/{len(chapters_with_edits) if chapters_with_edits else 0}")

        phase3_metadata["steps_executed"].append(4)
        phase3_metadata["total_chapter_cards_generated"] = chapter_card_count
        step_timings["step4_chapter_cards"] = round(time.time() - step_start, 2)
        save_codex(codex, codex_path)
        print(f">>> Step 4 complete ({step_timings['step4_chapter_cards']:.1f}s)")
    ...
    if 5 in steps_to_run:
        step_start = time.time()
        print(f"\n{'='*60}")
        print("STEP 5: Audio (Qwen3-TTS Direct Inference)")
        print(f"{'='*60}")
        ...
        phase3_metadata["steps_executed"].append(5)
        ...
        step_timings["step5_audio"] = round(time.time() - step_start, 2)
        save_codex(codex, codex_path)
        print(f"\n>>> Step 5 complete ({step_timings.get('step5_audio', 0):.1f}s):")
    ...
    if 6 in steps_to_run:
        print(f"\n>>> Step 6 (Video) is currently disabled")
...
def main():
    ...
    parser.add_argument(
        ...
        help="Run specific steps (0: Characters, 1: Locations, 2: Scenes, 3: Thumbnails, 4: Audio, 5: Chapter Cards, 6: Video)"
    )
    parser.add_argument(
        "--timeout",
        type=int,
