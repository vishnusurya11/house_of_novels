...
# Events that should trigger a macOS system notification (Notification Center)
NOTIFY_EVENTS = {"Stop", "Notification", "PermissionRequest"}
...
def send_macos_notification(title, message):
    ...
    if platform.system() != "Darwin":
        return False
    try:
        # Escape double quotes in message to prevent osascript injection
...
def main():
    ...
    try:
        ...
        # Step 5: Send macOS system notification for events where user needs to act
        if event_name in NOTIFY_EVENTS:
            notification_type = input_data.get("notification_type", "")
            if event_name == "Notification" and notification_type == "permission_prompt":
                send_macos_notification("Claude Code", "Permission needed - waiting for your approval")
            elif event_name == "Notification" and notification_type == "idle_prompt":
                send_macos_notification("Claude Code", "Waiting for your input")
            elif event_name == "PermissionRequest":
                send_macos_notification("Claude Code", "Permission needed - waiting for your approval")
            elif event_name == "Stop":
                send_macos_notification("Claude Code", "Finished - waiting for your input")
